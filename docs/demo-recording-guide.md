# Demo Recording Guide

Record the React/Vite product as the primary public demo. The static demo is a compatibility fallback, not the main story.

## Setup

Run:

```powershell
npm.cmd run check
npm.cmd run dev -- --port 5173
```

Open:

```text
http://127.0.0.1:5173/
```

Use a clean desktop browser around 1440 x 900. The responsive layout also supports a short mobile proof clip.

## Three-minute Shot List

1. Click **Reset story** and introduce Community Spirit as a community life RPG engine grounded in real spatial operations.
2. Show why the basketball Community Pulse matches Lin.
3. Use **Run next step** to join and reveal the real task, participant place, and route.
4. Run check-in and show XP, badge, Spirit Points, Season Arc, and shared-goal progress.
5. Claim, activate, and redeem the sports benefit. Pause on the synthetic resident benefit pass and wallet ledger.
6. Let the seventh step switch to World Ops. Show recommendations, opens, joins, check-ins, space activation, and commerce conversion.
7. Briefly demonstrate Pulse lifecycle controls, then return to `matching`.
8. Close on the structured robot patrol export as the future reality layer.

## Voiceover Spine

```text
Community Spirit turns real community spaces into a living RPG.
Community Pulse finds the right resident for the right real-space moment.
Joining creates a task and route; arriving grows resident identity and a shared community goal.
Spirit Points unlock useful benefits with a traceable resident pass and wallet ledger.
The same event becomes a measurable World Ops funnel for the property team.
This prototype uses fictional or synthetic data only.
```

## Capture Rules

- Follow the seven-step controller rather than free-clicking through unrelated features.
- Keep lifecycle controls brief so the resident story remains central.
- Do not show terminals, tokens, browser profiles, or private files.
- Do not imply real payments, resident identities, merchants, access systems, cameras, or robots are connected.
- Use `pitch/demo-script.md` for the full timed narration.
